...
static struct rtnl_link *rtnl_msg_handlers[RTNL_FAMILY_MAX + 1]; //rtnl_register注册的proto msgtype分布到该结构中，每个对应的函数见pktsched_init
...
//tc rtnl的dumpit函数在pktsched_init
...
void rtmsg_ifinfo(int type, struct net_device *dev, unsigned change)
{
	...
	rtnl_notify(skb, net, 0, RTNLGRP_LINK, NULL, GFP_KERNEL);//通过dev事件通知链RTM_NEWLINK通知给应用程序，该dev注销了
	return;
errout:
	if (err < 0)
...
//nlh是在rtnetlink_rcv函数中从SKB中解析出来的nlmsghdr
static int rtnetlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	return doit(skb, nlh, (void *)&rta_buf[0]);//tc 见//tc rtnl的doit函数在pktsched_init
}
...
//调用rtnetlink_init完成路由netlink的初始化
void __init rtnetlink_init(void)
{
	...
	rtnl_register(PF_UNSPEC, RTM_GETROUTE, NULL, rtnl_dump_all); //如果收到family是PF_UNSPEC， type是RTM_GETROUTE，执行rtnl_dump_all      rtnetlink_rcv_msg
}
