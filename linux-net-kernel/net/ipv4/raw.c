...
static struct raw_hashinfo raw_v4_hashinfo = { //通过prot-.hash中的raw_hash_sk把raw套接字struct sock添加到该hashinfo中
...
static int raw_v4_input(struct sk_buff *skb, struct iphdr *iph, int hash)
{
	...
	while (sk) {
		delivered = 1;
		if (iph->protocol != IPPROTO_ICMP || !icmp_filter(sk, skb)) { //协议号是 IPPROTO_ICMP(我们当前正处理的情况)时，则还需要判断该icmp类型的报文是否是被过滤掉的，如果是，则不处理
