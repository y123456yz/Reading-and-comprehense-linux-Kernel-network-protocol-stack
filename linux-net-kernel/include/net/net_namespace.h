...
struct neAAt {
	...
	struct hlist_head 	*dev_name_head; //dev name链表，查找dev的时候可以根据eth0做键值找  struct net_dev
	struct hlist_head	*dev_index_head; //dev索引号，创建dev的时候，都会有一个，不会重复
...
struct net {
	...
	struct list_head 	dev_base_head; //见list_netdevice            一般基于MAC地址或者设备类型就用该链表查找，基于接口名或者索引使用下面两个查找
       /*
         * 将设备名作为散列键的链表
         *///dev_get_by_name
	struct hlist_head 	*dev_name_head;//dev name链表，查找dev的时候可以根据eth0做键值找  struct net_dev   
